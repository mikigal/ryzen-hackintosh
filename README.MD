...
## How to use
  ...
  6. Read [**informations**](#Hardware-compatibility) about required changes for some hardware
  7. Boot it!
...
## BIOS Settings
  ...
  - **SVM**: Enabled if you want to use virtual machines
  - **Above 4G Decoding**: Enabled (If you have this option you **MUST** remove `npci=0x2000` remove from `boot-args` in `config.plist`)
...
Most builds with **17h and 19h (Ryzens, Athlon 2xxGE)** CPUs with [**macOS compatible peripherals**](https://dortania.github.io/Anti-Hackintosh-Buyers-Guide/CPU.html) should work with EFI from this repository. \
**Support for 16h (FX) and Threadripper CPUs is not covered here.**
...
If you experience issues with network connection, then you probably have another ethernet chipset. Click [**here**](https://dortania.github.io/OpenCore-Install-Guide/ktext.html#ethernet) for informations about kexts for ethernet cards. Remember to remove `RealtekRTL8111.kext` before use another kext.

By default enabled is PAT patch made by Shaneee - it improves GPU performance but can cause some issues. It can break HDMI audio and make builds with NVIDIA GPU unbootable. You can switch patch with better compatibility (but worse performance). Click [**here**](#PAT-patch-information) for more informations.
...
Only compatible software for virtual machines are Parallels Desktop (only up to 13.1, newer version require AppleHV) and VirtualBox (it works much worse than Parallels). There's also VMWare Fusion 10, but it's totally broken on Big Sur, on Catalina it needs [**this workaround**](https://posts.boy.sh/vmware-fusion-catalina) \
Also Docker does not work - you have to use Docker Toolbox instead, but it does not have all Docker's features.
...
Same problem will exist while trying to start installed Parallels - you can run it every time with the same way as installer or use my launcher (it simply starts Parallels with needed parameter) - you can get it [**here**](/Resources/Parallels%20Desktop%20Launcher.app.zip)
...
If Parallels does not see USB device reconnecting to another port usually solve this issue.
